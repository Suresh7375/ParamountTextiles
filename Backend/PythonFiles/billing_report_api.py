...
def create_invoice(output_path, bill_data):
    print("Received data1:", bill_data)
    ...
    invoice_details = Paragraph(
        f"""<para align=right>
            ...
            <font name="Helvetica" size="9" color="#{BRAND_TEXT.hexval()[2:]}">{bill_data["invoice"]}</font><br/>
            <font name="Helvetica-Bold" size="9" color="#{BRAND_TEXT.hexval()[2:]}">Date:</font> 
            <font name="Helvetica" size="9" color="#{BRAND_TEXT.hexval()[2:]}">{bill_data["date"]}</font><br/>
            <font name="Helvetica-Bold" size="9" color="#{BRAND_TEXT.hexval()[2:]}">Our Ref:</font> 
            <font name="Helvetica" size="9" color="#{BRAND_TEXT.hexval()[2:]}">{bill_data["ourRef"]}</font><br/>
            <font name="Helvetica-Bold" size="9" color="#{BRAND_TEXT.hexval()[2:]}">Your Ref:</font> 
            <font name="Helvetica" size="9" color="#{BRAND_TEXT.hexval()[2:]}">{bill_data["yourRef"]}</font>
        </para>""",
        styles["Normal"]
    )
    ...
    #     ["GSTIN:", ""]
    # ]
    bill_to = bill_data.get("billTo", {})
    bill_to_data = [
        ["Name:", f"M/S. {bill_to.get('name', '')}"],
        ["Address:", bill_to.get('address', '')],
        ["Area:", bill_to.get('area', '')],
        ["City:", bill_to.get('city', '')],
        ["PIN:", bill_to.get('pin', '')],
        ["GSTIN:", bill_to.get('gstin', '')]
    ]
    ship_to = bill_data.get("shipTo", {})

    ship_to_data = [
        ["Name:", f"M/S. {ship_to.get('name', '')}"],
        ["Address:", ship_to.get('address', '')],
        ["Area:", ship_to.get('area', '')],
        ["City:", ship_to.get('city', '')],
        ["PIN:", ship_to.get('pin', '')],
        ["GSTIN:", ship_to.get('gstin', '')]
    ]
    ...
    table_header = ["Sl. No.", "Description", "HSN/SAC", "Qty", "Unit", "Rate", "Amount", "GST%", "GST Amt",
                    "Total"]
    # items = [
    ...
    # ]
    product_details = bill_data.get("productDetails", {})
    items = [
        [
            str(p['slNo']),
            p['description'],
    ...
    summary_data = [
        [Paragraph("Sub Total", ParagraphStyle('SubTotal', parent=normal_style, fontName='Helvetica')),
         Paragraph(f"{bill_data['subTotal']}.00", ParagraphStyle('SubTotalVal', parent=normal_style1, fontName='Helvetica'))],
        [Paragraph("GST @ 7%", ParagraphStyle('Tax', parent=normal_style, fontName='Helvetica')),
         Paragraph(f"{bill_data['gstTotal']}.00", ParagraphStyle('TaxVal', parent=normal_style1, fontName='Helvetica'))],
        [Paragraph("<b>Total</b>",
                   ParagraphStyle('TotalBold', parent=normal_style, fontName='Helvetica-Bold', fontSize=10)),
         Paragraph(f"<b>{bill_data['total']}.00</b>",
                   ParagraphStyle('TotalBoldRight', parent=normal_style1, fontName='Helvetica-Bold', fontSize=10))]
    ]
    ...
    amount_in_words = num2words(bill_data['total'], to='cardinal', lang='en').title()
    ...
    amount_words_para = Paragraph(
        ...
        <font name="Helvetica" size="9" color="#{BRAND_TEXT.hexval()[2:]}">Singapore Dollars {amount_in_words} Only</font></para>""",
        ParagraphStyle(
            name='Words',
            fontSize=9,
            leading=12,
    ...
    bank_details_data = bill_data.get("bankDetails", {})
    bank_data = [
        [Paragraph("Account Name:", bank_label_style),
         Paragraph(bank_details_data['accountName'], bank_data_style),
         Paragraph("A/C No.:", bank_label_style),
         Paragraph(bank_details_data['accountNo'], bank_data_style)],
        [Paragraph("Bank Name:", bank_label_style),
         Paragraph(bank_details_data['bankName'], bank_data_style),
         Paragraph("SWIFT Code:", bank_label_style),
         Paragraph(bank_details_data['swiftCode'], bank_data_style)],
        [Paragraph("IFSC Code:", bank_label_style),
         Paragraph(bank_details_data['ifscCode'], bank_data_style), "", ""]
    ]
    ...
    # Signature Paragraph (right-aligned)
    signature_text = Paragraph(
        f"""For <b>Paramount Clothing</b><br/><br/>
        <font name="Helvetica-Bold" size="9" color="#{BRAND_PRIMARY.hexval()[2:]}">Authorized Signatory</font>""",
        ParagraphStyle(
            name='Sign',
            alignment=TA_RIGHT,
            fontSize=9,
    ...
    # Right-align by wrapping in full-width table
    ...
    thank_you_note = Paragraph(
        f"""<font name="Helvetica-Oblique" size="9" color="#{BRAND_SECONDARY.hexval()[2:]}">
        Thank you for your business!</font>""",
        ParagraphStyle(
            name='ThankYou',
            alignment=TA_CENTER,
            fontSize=9,
    ...
    # Center-align by wrapping in full-width table
...
# Define the API endpoint for creating invoices
...
def receive_data():
    try:
        ...
        invoice_file = f"paramount_invoice_{data['invoice']}"".pdf"
        create_invoice(invoice_file, data)

        # Here you could return the file or a success message with file location
        return jsonify({
            "message": 0,
            "invoice_path": f"http://localhost/PythonProject/{invoice_file}"
        }), 200
...
# API endpoint to download the generated invoice
@app.route('/api/download-invoice', methods=['GET'])
def download_invoice():
    try:
        # Path to the invoice PDF
        invoice_file = "paramount_invoice_modern.pdf"
